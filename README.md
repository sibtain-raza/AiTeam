...
- **Scaling scope to the goal works in both directions, verified across three real runs.** A literal "make a page that says Hi" run and a static pricing-page run both correctly got `BE: 0, OPS: 0` from the architect — both engineers skipped without starting a Claude Code session (`ClaudeCodeAgent.on_messages()` short-circuits before `ClaudeAgentOptions`/`query()` even get constructed) — while QA still did real, substantive work: on the pricing-page run it caught a genuinely broken production build (a Vite/Vitest config mismatch), a real `jest-axe` accessibility violation, and an off-by-one CSS breakpoint, all three fixed and re-verified in a real rework loop. Then, on an actually-complex goal (a full URL shortener with custom aliases and click analytics — the same task referenced above), the architect correctly assigned all three roles substantial, non-zero budgets (`FE: 35, BE: 45, OPS: 40`) and produced a proportionally deep design (SSRF protection, partitioned click-analytics tables, Redis Streams ingestion, full Terraform/ECS/CloudFront) — confirming the fix doesn't just skip engineers by default, it skips them only when a role genuinely has nothing to do.
- **QA catches real, substantive defects on a full build, not just toy ones.** On that same URL-shortener run, a fresh QA pass (25→45-turn budget, real `Read`/`Grep`/`Bash` access) found: a backend alias-validation regex that silently rejected hyphens/underscores while the frontend's own validation and the PRD both required them (a genuine FE/BE contract break); a CI workflow and RUNBOOK that both referenced an `npm run test:ci` script neither `package.json` actually defined, meaning **every CI run failed immediately and no code could ever reach build/deploy through the documented pipeline**; `npm run lint` broken in both projects for two unrelated root causes (missing `eslint` dependency in the backend, missing config file in the frontend); and a live-looking, fully-generated JWT secret and DB credentials sitting in a root `.env` file with no `.gitignore` anywhere except `frontend/` — a real secret one `git add` away from being committed. None of these were manufactured or planted; QA found them by actually reading the files and running the actual build/lint/test commands.
- **A real operational hazard, discovered the hard way: never run a second `--resume` against a checkpoint without confirming the first invocation has actually exited.** During the URL-shortener run above, the account's Claude Code session limit was hit mid-run; a manual `--resume` appeared to fail immediately (printed a session-limit traceback) and was assumed dead, so a second, separate `--resume` against the *same* checkpoint was started shortly after. Both turned out to still be alive and both ran concurrently against the same workspace for several minutes — visible afterward as `qa_engineer` appearing twice in a row in the transcript with no engineer turn in between, which is impossible under normal graph routing. A dedicated fresh QA re-check (a standalone script instantiating `ClaudeCodeAgent` directly, bypassing GraphFlow entirely, pointed at the live workspace) confirmed the specific files involved hadn't actually been corrupted this time — no conflict markers, consistent cross-file config, passing tests — but that was fortunate overlap in what each process happened to fix, not a guarantee. `--resume` has no lock file or other guard against two processes targeting the same checkpoint; verify via `ps` (not just the printed output) that an earlier invocation has fully exited before starting another one against the same checkpoint path.
...
- **Cross-model QA/UAT judging.** Today QA and UAT run the *same* Claude model as the engineers whose work they grade (`LOOPER_CODE_MODEL` is uniform by design), which risks correlated blind spots — a mistake pattern the model family tends to make is also the pattern it tends not to notice when reviewing. Moving just the judge roles to a different model family would add genuine independent review. It's real surgery, not a patch: the pipeline is deliberately built on the `claude` CLI's OAuth login (no API keys), so a second vendor means reviving something like the dead `config.py` provider path for those roles — and it changes the project's "no API keys needed" story, which is why it's listed here rather than done. (Pairs naturally with the "split billing by role need" item above: UAT is already a tool-less role that could move off the CLI.)
