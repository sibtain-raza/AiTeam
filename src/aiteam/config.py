...
Selection via environment variables:
  AITEAM_PROVIDER  anthropic (default) | openai
  AITEAM_MODEL     optional model override
...
def make_model_client() -> ChatCompletionClient:
    provider = os.environ.get("AITEAM_PROVIDER", "anthropic").lower()
    model = os.environ.get("AITEAM_MODEL") or DEFAULT_MODELS.get(provider)
    ...
    raise ValueError(
        f"Unknown AITEAM_PROVIDER={provider!r} — expected 'anthropic' or 'openai'"
    )
